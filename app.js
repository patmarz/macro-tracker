/* Macro Tracker v1.1.2 */
const APP_VERSION = "v1.1.2";
const STORAGE_KEY = "macro-tracker:data";
const UNDO_STACK_KEY = "macro-tracker:undoStack";
...
// --- Settings/theme (unchanged from 1.1.1) ---
function defaultSettings(){ return {targets:{cal:2000,p:150,c:200,f:70}, theme:"auto", showRemaining:false}; }
...
// --- Data layer ---
function loadRaw(){ try{ return JSON.parse(localStorage.getItem(STORAGE_KEY) || "null"); }catch(e){ return null; } }
function loadMeals(){
  const raw = loadRaw();
  if(!raw){ return []; }
  if(raw.schemaVersion !== SCHEMA_VERSION){
    ...
    if(raw.meals){ raw.schemaVersion = SCHEMA_VERSION; localStorage.setItem(STORAGE_KEY, JSON.stringify(raw)); return raw.meals; }
    return [];
  }
  return raw.meals || [];
}
function saveMeals(meals){ localStorage.setItem(STORAGE_KEY, JSON.stringify({schemaVersion:SCHEMA_VERSION, meals})); }
function pushStack(key, action){ const arr = JSON.parse(localStorage.getItem(key) || "[]"); arr.unshift(action); localStorage.setItem(key, JSON.stringify(arr.slice(0,20))); }
function popStack(key){ const arr = JSON.parse(localStorage.getItem(key) || "[]"); const item = arr.shift(); localStorage.setItem(key, JSON.stringify(arr)); return item; }
function clearStack(key){ localStorage.removeItem(key); }

// --- Helpers ---
function computeTotals(mealsForDay){ return mealsForDay.reduce((a,m)=>({cal:a.cal+(+m.calories||0),p:a.p+(+m.protein||0),c:a.c+(+m.carbs||0),f:a.f+(+m.fat||0)}),{cal:0,p:0,c:0,f:0}); }
...
function render(){
  ...
  const s = loadSettings();
  const tgt = s.targets;
  const pairs = [["kpiCal", t.cal, tgt.cal],["kpiP", t.p, tgt.p],["kpiC", t.c, tgt.c],["kpiF", t.f, tgt.f]];
...
// --- CRUD, Undo/Redo, Presets, Copy Yesterday, Import/Export ---
// (same as 1.1.1, IDs kept)
...
function onSave(e){
  ...
  for(const f of numFields){
    if(m[f] < 0 || !isFinite(m[f])){ alert("Please enter only non-negative numeric values."); return; }
  ...
  const entry = { id: form.dataset.editing || crypto.randomUUID(), ...m };
  const meals = loadMeals();
  const idx = meals.findIndex(x => x.id === entry.id);
  if(idx>=0){ const prev = {...meals[idx]}; meals[idx] = entry; pushStack(UNDO_STACK_KEY, {type:"edit", before:prev, after:entry}); clearStack(REDO_STACK_KEY); }
  else { meals.push(entry); pushStack(UNDO_STACK_KEY, {type:"add", item:entry}); clearStack(REDO_STACK_KEY); }
  saveMeals(meals);
  form.reset(); form.dataset.editing = ""; $("#date").value = $("#filterDate").value; render();
}
function onDelete(e){
  ...
  pushStack(UNDO_STACK_KEY, {type:"delete", item:removed}); clearStack(REDO_STACK_KEY);
  saveMeals(meals); render();
}
function onEdit(e){
  const id = e.target.dataset.edit;
  const item = loadMeals().find(m => m.id === id);
  if(!item) return;
  $("#date").value = item.date; $("#meal").value = item.meal;
  $("#calories").value = item.calories; $("#protein").value = item.protein;
  $("#carbs").value = item.carbs; $("#fat").value = item.fat;
  $("#notes").value = item.notes; $("#category").value = item.category || ""; $("#time").value = item.time || "";
  $("#form").dataset.editing = item.id; $("#meal").focus();
}
function applyActionReverse(action){
  const meals = loadMeals();
  if(action.type === "delete"){ meals.push(action.item); }
  else if(action.type === "add"){ const idx = meals.findIndex(m => m.id === action.item.id); if(idx>=0) meals.splice(idx,1); }
  else if(action.type === "edit"){ const idx = meals.findIndex(m => m.id === action.after.id); if(idx>=0) meals[idx] = action.before; }
  else if(action.type === "bulkAdd"){ const left = meals.filter(m => !action.items.some(x => x.id===m.id)); saveMeals(left); render(); return; }
  saveMeals(meals);
}
function applyActionForward(action){
  const meals = loadMeals();
  if(action.type === "delete"){ const idx = meals.findIndex(m => m.id === action.item.id); if(idx>=0) meals.splice(idx,1); }
  else if(action.type === "add"){ meals.push(action.item); }
  else if(action.type === "edit"){ const idx = meals.findIndex(m => m.id === action.before.id); if(idx>=0) meals[idx] = action.after; }
  else if(action.type === "bulkAdd"){ saveMeals(meals.concat(action.items)); render(); return; }
  saveMeals(meals);
}
function onUndo(){ const a = popStack(UNDO_STACK_KEY); if(!a) return; applyActionReverse(a); pushStack(REDO_STACK_KEY, a); render(); }
function onRedo(){ const a = popStack(REDO_STACK_KEY); if(!a) return; applyActionForward(a); pushStack(UNDO_STACK_KEY, a); render(); }
...
function addPresetFromCurrent(){ const p = currentMealFromForm(); const presets = loadPresets(); presets.unshift(p); savePresets(presets.slice(0,25)); renderPresets(); closeMenu(); }
function applyPreset(idx){
  ...
  $("#category").value = p.category || ""; $("#time").value = p.time || "";
  closeMenu();
}
function renderPresets(){
  const sel = $("#presetPicker"); if(!sel) return;
...
function copyYesterday(){
  ...
  saveMeals(meals.concat(y)); pushStack(UNDO_STACK_KEY, {type:"bulkAdd", items:y}); clearStack(REDO_STACK_KEY);
  render(); closeMenu();
}
...
function toCSV(meals){
  ...
  const rows = meals.map(m => headers.map(h => {
    let v = m[h] ?? ""; v = (""+v).replace(/"/g,'""'); return `"${v}"`;
  }).join(","));
  return headers.join(",") + "\n" + rows.join("\n");
}
...
function previewImport(items){
  ...
  let newCount=0, updateCount=0; items.forEach(it => ids.has(it.id) ? updateCount++ : newCount++);
  ...
  showModal("#importModal"); window.__pendingImport = items;
}
function commitImport(strategy){
  const items = window.__pendingImport || []; const meals = loadMeals(); const map = new Map(meals.map(m => [m.id, m]));
  if(strategy === "replaceById"){ for(const m of items){ map.set(m.id, m); } }
  else { for(const m of items){ if(!map.has(m.id)) map.set(m.id, m); } }
  saveMeals(Array.from(map.values())); delete window.__pendingImport; closeModal("#importModal"); render();
}
function importJSONFile(file){
  const reader = new FileReader();
  reader.onload = () => { try{ const obj = JSON.parse(reader.result); if(obj && Array.isArray(obj.meals)){ previewImport(obj.meals); } else throw new Error("Not a Macro Tracker JSON payload."); }catch(err){ alert("Import failed: " + err.message); } };
  reader.readAsText(file);
}
function importCSVFile(file){
  const reader = new FileReader();
  reader.onload = () => { try{ const items = parseCSV(reader.result); previewImport(items); }catch(err){ alert("CSV import failed: " + err.message); } };
  reader.readAsText(file);
}
...
function doUpdateNow(){ if(swReg && swReg.waiting){ swReg.waiting.postMessage({type:"SKIP_WAITING"}); } }

// --- Settings modal ---
function openSettings(){
  const s = loadSettings();
  $("#targetCal").value = s.targets.cal; $("#targetP").value = s.targets.p; $("#targetC").value = s.targets.c; $("#targetF").value = s.targets.f;
  $("#themeSel").value = s.theme || "auto"; $("#showRemaining").checked = !!s.showRemaining;
  showModal("#settingsModal"); closeMenu();
}
function saveSettingsFromUI(){
  ...
  s.theme = $("#themeSel").value; s.showRemaining = $("#showRemaining").checked;
  saveSettings(s); applyTheme(); closeModal("#settingsModal"); render();
}

// --- Init ---
document.addEventListener("DOMContentLoaded", ()=>{
  $("#ver").textContent = APP_VERSION; applyTheme();
  $("#filterDate").value = todayISO(); $("#date").value = todayISO();
  ...
  $("#importCommit").addEventListener("click", ()=>{ commitImport($("#mergeStrategy").value); });
